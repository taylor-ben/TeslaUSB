...
- [x] Feedback model: JSON for AJAX + flash banners; live-poll views. **(audit-complete 2026-07-22 — uniform across ALL routes, not just media. Rust: every error-capable handler returns `Result<_, ApiError>`; `error.rs` `IntoResponse` serializes the D2 §1 envelope `{"error":{"code","message"}}` (400/404/409/413/422/500/503/507); even the `/api` fallback (`api_not_found`) returns it. Status/live-poll endpoints (health/storage/metrics, gadget+wifi status, jobs SSE, jobs/failed, media-events SSE) are deliberately infallible `200` + degrade-to-null so polling never flaps. SPA: `api/client.ts` centralizes envelope parsing → typed `ApiError(status,code,message)`; all mutation callers (media, settings/`put_setting`, wifi connect/forget, timezone, delete-clip, chime-scheduler) surface it as a status/flash region (`setPrefError`/`setWifiMsg`/`setTzError`/`setDeleteFail`/…). "flash+redirect for forms" = N/A on B-1 (JSON-only SPA, no server-rendered forms — see §5). Verified by anti-pattern grep (no bare `StatusCode`/hand-rolled error shape in production) + a fresh-context adversarial audit = zero counterexamples across both the route surface and the SPA screens.)**
