...
| `GET /api/trips/:id/clips` | Trip map route-click video | Trip's dashcam clips (map route-click video); clips whose time window overlaps the trip, ordered by started_at ascending; non-paginated; same item shape as `GET /api/clips`. | `clips`,`angles` |
...
| `GET /api/clips/:id/telemetry?camera=front` | Video overlay HUD | Bare JSON array of telemetry samples for HUD playback sync (`time`, `speedMps`, `gear`, `steeringAngle`, `blinkerLeft`, `blinkerRight`, `brakeApplied`, `acceleratorPedalPosition`, `autopilotState`) parsed on demand from archived clip SEI. | Pi archive file bytes + SEI parser |
...
| `GET /api/clips/:id/telemetry?camera=front` | Read-only telemetry endpoint for HUD overlays. Returns `200 application/json` with a bare array of `{ time, speedMps, gear, steeringAngle, blinkerLeft, blinkerRight, brakeApplied, acceleratorPedalPosition, autopilotState }` where `time` is seconds, `speedMps` is m/s, `gear` is `0=P/1=D/2=R/3=N`, `steeringAngle` is raw Tesla value, pedal is `0..1`, booleans are lamp/pedal states, and `autopilotState` is `0..3` (`NONE/SELF_DRIVING/AUTOSTEER/TACC`). Unknown clip/camera is `404`. Returns `[]` when the resolved angle is non-archive, clip has no Tesla SEI, parse/read fails, or file exceeds the size cap. Parsing is Pi-side archive-only (never live USB image), serialized (`permit=1`) and size-capped to protect memory/recording-path contention. |
